function [Result] = fitPattern(Obj,Args)
arguments
    Obj;
    ...
    Args.HistRotEdges            = (-20:0.1:20);  % rotation or [min max] rotation that require to ?
...
RefCat = AstroCatalog({Obj.Cats(1).getCol({Args.ColNameRefX,Args.ColNameRefY})},'ColNames',{Args.ColNameRefX,Args.ColNameRefY});
% match catalogs
if nargout>1
    OutputArgs = cell(1,3);
    Nargs = 3;
...
if ~isempty(Args.MaxRefMag)
    RefMag = Obj.Cats(1).getCol(Args.RefMagCol);
    FlagMaxMag = RefMag<=Args.MaxRefMag;
    RefCat.Catalog = RefCat.Catalog(FlagMaxMag,:);
end

for IndCat = 1:numel(Obj.Cats)
    
    [OutputArgs{1:Nargs}] = imUtil.patternMatch.match_scale_rot_shift(RefCat.Catalog, Obj.Cats(IndCat).getCol({Args.ColNameX,Args.ColNameY}),...
        'CatColX',1, 'CatColY',2,...
        'RefColX',1, 'RefColY',2,...
        'Scale',1,...

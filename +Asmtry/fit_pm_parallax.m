...
DefV.Plx    = true;
DefV.ModelJD= [];
DefV.Plx_is_Given= false;
DefV.GivenPlx = [];
DefV.RefEpoch = 2451545.0;% GAIA 2457206.375
...
Ha = [ones(Nobs,1), (JD-JDref)./JYear];
Ya = OffRA(:,1);
Yae= OffRA(:,2);

% design matrix for Dec
Hd = [ones(Nobs,1), (JD-JDref)./JYear];
Yd = OffDec(:,1);
Yde= OffDec(:,2);

if (InPar.Plx && ~InPar.Plx_is_Given)
...
Y = [Ya; Yd] - InPar.GivenPlx/1000/3600/180*pi * [(X.*sin(RA_t) - Y.*cos(RA_t)); (X.*cos(RA_t).*sin(Dec_t) + Y.*sin(RA_t).*sin(Dec_t) - Z.*cos(Dec_t))];
Ye= [Yae;Yde];
Na = length(Ya);
Nd = length(Yd);
...
%[~,~,Flag]=Util.fit.clip_resid(Resid,varargin);
...
if (isempty(InPar.ModelJD))
    ...
    % design matrix for RA + PM in RA
    Ha = [ones(Nmodel,1), (InPar.ModelJD-JDref)./JYear];

    % design matrix for Dec
    Hd = [ones(Nmodel,1), (InPar.ModelJD-JDref)./JYear];

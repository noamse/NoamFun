function plotSourceCurves(Obj,SourceInd,Args)
...
TS = Obj.getTimeSeriesField(SourceInd,{'X','Y','MAG_PSF'});
...
PMX = Obj.PMX(:,SourceInd);
PMY = Obj.PMY(:,SourceInd);
ax1=subplot(3,1,1);
plot(Obj.JD-2450000,TS(:,3),'.','Color',Args.Color);
ylabel('I [mag]','interpreter','latex')
set(gca,'YDir','reverse')
ax2= subplot(3,1,2);
plot(Obj.JD-2450000,TS(:,1),'.','Color',Args.Color);
hold on;
plot(Obj.JD-2450000,H*PMX);
ylabel('X [pix]','interpreter','latex')
hold off;
ax3= subplot(3,1,3);
plot(Obj.JD-2450000,TS(:,2),'.','Color',Args.Color);
hold on;
plot(Obj.JD-2450000,H*PMY);
...
BMag = timeSeries.bin.binningFast([Obj.JD(~OutMag)-2450000, TS(~OutMag,3)], Args.TimeBinSize,[NaN NaN],{'MidBin', @nanmean, @tools.math.stat.rstd,@numel});
FlagMag = ~(BMag(:,2)==0 | isnan(BMag(:,2)));
errorbar(BMag(FlagMag,1),BMag(FlagMag,2),BMag(FlagMag,3)./sqrt(BMag(FlagMag,4)),'.')
%hold on;
%plot(Obj.JD-2450000,H*CM.pm_x(:,Sind));
...
DX = (H*PMX-TS(:,1));
OutX=isoutlier(DX);
Bx = timeSeries.bin.binningFast([Obj.JD(~OutX)-2450000, TS(~OutX,1)], Args.TimeBinSize,[NaN NaN],{'MidBin', @nanmean, @tools.math.stat.rstd,@numel});
Bx(Bx(:,3)==0,3) = Inf;
...
DY = (H*PMY-TS(:,2));
OutY=isoutlier(DY);
By = timeSeries.bin.binningFast([Obj.JD(~OutY)-2450000, TS(~OutY,2)], Args.TimeBinSize,[NaN NaN],{'MidBin', @nanmean, @tools.math.stat.rstd,@numel});
By(By(:,3)==0,3) = Inf;
...
if Args.PlotPlx 
    ...
    PosPlx = H*Obj.PMPlx(1:5,SourceInd);
    ...
    HpmSrc = Obj.designMatrixPM('JD',Bx(FlagX,1),'JD0',0);
    ...
    PosPlx = Hbin*Obj.PMPlx(1:5,SourceInd);
    XPlx = PosPlx(1:numel(PosPlx)/2);
    YPlx = PosPlx(numel(PosPlx)/2+1:end);

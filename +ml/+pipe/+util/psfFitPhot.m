function [ResultObj, Result] = psfFitPhot(Obj, Args)
    ...
    for Iobj=1:1:Nobj
        ...
        if ~isempty(PSF)
            ...
            % calculate magnitude
            if Args.UpdateCat
                ResultObj(Iobj).CatData.insertCol(double([Result.X, Result.Y, Result.Flux, Result.Mag, Result.MagErr, Result.Chi2./Result.Dof,Result.SNm,Result.Back]),...
                                        Inf,...
                                        {'X',      'Y',      'FLUX_PSF',  'MAG_PSF', 'MAGERR_PSF', 'PSF_CHI2DOF','SN','Back'},...
                                        {'pix',    'pix',    '',          'mag',     'mag',        '',            '',   ''});
            end
        else % empty PSF
            % PSF is empty - skip
